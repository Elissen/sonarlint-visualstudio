      ...
      <NuGetPackage Id=""SonarAnalyzer.CSharp"" Version=""6.4.0.3322"" />
        ...
        public async Task GetNotificationEventsAsync_CallsExpectedUrl()
        {
            ...
            await Method_CallsTheExpectedUri(expectedUri, @"{""events"": [] }",
                (c, co, t) => c.GetNotificationEventsAsync(co, request, t));
        }
        ...
        {
            // Arrange
            var sqConnection = new ConnectionRequest { ServerUri = new Uri("http://mysq.com/") };
...
""date"":""2017-09-14T10:55:19+0200""}]}");
            ...
            var result = await client
                .GetNotificationEventsAsync(sqConnection, request, CancellationToken.None);
            ...
            HttpStatusCode returnedStatusCode)
        {
            ...
            // Act
            return await client.GetNotificationEventsAsync(sqConnection, request, CancellationToken.None);
        }
        ...
        private static SonarQubeClient SetupClientWithHttpResponse(HttpStatusCode code, string responseText)
        {
            ...
            return new SonarQubeClient(httpHandler.Object, TimeSpan.FromSeconds(10));
        }
