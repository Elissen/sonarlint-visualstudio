...
namespace SonarQube.Client.Services
{
    public class SonarQubeClient : ISonarQubeClient
    {
        ...
        public Task<Result<NotificationsResponse[]>> GetNotificationEventsAsync(ConnectionRequest connection,
            NotificationsRequest request, CancellationToken token)
        {
            const string NotificationsAPI = "api/developers/search_events"; // Since 6.6;  internal
            var query = AppendQueryString(NotificationsAPI, "?projects={0}&from={1}",
                        request.ProjectKey, ToJavaTimeFormat(request.EventsSince)); // TODO: should handle optional params
